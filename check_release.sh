...
dotnet build ../ZL.Gear.Full.sln -v q
if [ $? -ne 0 ]; then
    echo "❌ 编译失败！请检查代码错误。"
    exit 1
fi
...
# 2. 核心单元测试（含 ScenarioDemoLibraryTests 运行时：Core Showcase PASS / Timeout FAIL 等）
echo "[2/${TOTAL_STEPS}] 正在运行单元测试..."
dotnet test ../ZL.Gear.Drivers/ZL.Gear.Drivers.sln --no-build -v q --settings ../ZL.Gear.Drivers/ZL.Gear.Drivers.Tests/test.runsettings
if [ $? -ne 0 ]; then
...
dotnet build ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release -v q
if [ $? -ne 0 ]; then
    echo "❌ ConsoleApp 编译失败！"
    exit 3
fi
# 运行完整集成测试场景（--no-build 避免重复编译，见 docs/141 §10.3）
dotnet run --project ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/Scenarios/FullIntegrationTest.json --verbose > /tmp/zlgear_test.log 2>&1
...
dotnet run --project ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/Scenarios/MoatSmokeTest.json --verbose > /tmp/zlgear_moat.log 2>&1
...
# Release 构建禁用 DevMode；IndustryKit verify 与 TestSetup 同轨（docs/141 §10.5(4)）
export ZL_GEAR_LICENSE_TEST_MODE="${ZL_GEAR_LICENSE_TEST_MODE:-true}"
KIT_OUT=$(dotnet run --project ./demos/IndustryKit/ZL.Gear.Samples.Industry.Client/ZL.Gear.Samples.Industry.Client.csproj -c Release --no-build -- verify 2>&1)
...
dotnet run --project ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/Scenarios/Demo_Sync_SignalPair.json --verbose > /tmp/zlgear_sync.log 2>&1
...
dotnet run --project ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ../ZL.Gear.Demos/ZL.Gear.ConsoleApp/Scenarios/Demo_Sampling_Continuous.json --verbose > /tmp/zlgear_continuous.log 2>&1
