...
dotnet test ZL.Gear.sln --no-build -v q --settings ./ZL.Gear.Drivers.Tests/test.runsettings
if [ $? -ne 0 ]; then
...
dotnet build ./demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release -v q
if [ $? -ne 0 ]; then
    echo "❌ ConsoleApp 编译失败！"
    exit 3
fi
# 运行完整集成测试场景（--no-build 避免重复编译，见 docs/141 §10.3）
dotnet run --project ./demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ./demos/ZL.Gear.ConsoleApp/Scenarios/FullIntegrationTest.json --verbose > /tmp/zlgear_test.log 2>&1
...
dotnet run --project ./demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ./demos/ZL.Gear.ConsoleApp/Scenarios/MoatSmokeTest.json --verbose > /tmp/zlgear_moat.log 2>&1
...
dotnet run --project ./demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ./demos/ZL.Gear.ConsoleApp/Scenarios/Demo_Sync_SignalPair.json --verbose > /tmp/zlgear_sync.log 2>&1
...
dotnet run --project ./demos/ZL.Gear.ConsoleApp/ZL.Gear.ConsoleApp.csproj -c Release --no-build -- -s ./demos/ZL.Gear.ConsoleApp/Scenarios/Demo_Sampling_Continuous.json --verbose > /tmp/zlgear_continuous.log 2>&1
