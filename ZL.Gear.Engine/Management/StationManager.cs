...
namespace ZL.Gear.Engine.Management
{
    ...
    public class StationManager
    {
        ...
        public void StartAutoRun()
        {
            ...
            Task.Factory.StartNew(() => AutoRunLoop(_stationCts.Token), TaskCreationOptions.LongRunning);
        }
        ...
        private async Task AutoRunLoop(CancellationToken token)
        {
            ...
            try
            {
                await _deviceService.InitializeAllDevicesAsync(token: token);
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (_currentState == StationState.Idle)
                    {
                        ...
                        if (_triggerCondition != null && await _triggerCondition(token))
                        {
                            await SetStateAsync(StationState.Working);
                        }
                        else
                        {
                            await Task.Delay(100, token); // 避免 CPU 空转
                            continue;
                        }
                    }
                    ...
                    if (_currentState == StationState.Working)
                    {
                        ...
                        // 执行核心引擎
                        var result = await _executor.ExecuteAsync(steps, "", "", contextArgs, token);
