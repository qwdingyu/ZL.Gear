...
namespace ZL.Gear.Core.Models
{
    ...
    public static class StepKit
    {
        ...
        public static Dictionary<string, SensorSpec> ParseSensorSpecs(StepConfig step)
        {
            var specs = new Dictionary<string, SensorSpec>();
            if (step.Parameters != null && step.Parameters.TryGetValue("CustomerParam", out var cp) && cp != null)
            {
                ...
                {
                    var specNames = new[] { "座椅位置", "靠背位置", "座垫位置" };
                    ...
                    for (int i = 0; i < itemsToProcess; i++)
                    {
                        ...
                        {
                            specs[name] = new SensorSpec(name, lcl, ucl);
                        }
                    }
                }
            }
